...
export class PointSelector
    {
        public static NUM_LLOYD_RELAXATIONS: number = 2;
        ...
        /// </summary>
        public generateRandom(mapW: number, mapH: number, r: Random, numPoints: number, minSpacing: number = 3.0, mapEdgePadding: number = 10)
        {
            var points: Vector2f[] = [];
            for (let i = 0; i<numPoints; i++) {
                        var p = new Vector2f (r.range(mapEdgePadding, mapW - mapEdgePadding),
                            r.range(mapEdgePadding, mapH - mapEdgePadding) );

                //Check that it isnt too close to any other point
                let tooClose: boolean = false;
                for(let op of points)
                {
                    if(Vector2f.Distance(p,op) < minSpacing) { tooClose = true; break; }
                }
                if (!tooClose) { points.push(p); } //If not too close, add to list
            }
            return points;
        }

        // Generate points on a square grid
        public generateSquare(mapW: number, mapH: number, r: Random, numPoints: number) : Vector2f[]
        {
          let points: Vector2f[] = [];
          let N = Math.sqrt(numPoints);
          for (let x = 0; x < N; ++x) {
            for (let y = 0; y < N; ++y) {
                    points.push(new Vector2f((0.5 + x) / N * mapW, (0.5 + y) / N * mapH ));
        ...
        public generateHexagon(size: number, r: Random, numPoints: number): Vector2f[] {
          let points: Vector2f[] = [];
          let N = Math.sqrt(numPoints);
          for (var x = 0; x<N; x++) {
          for (var y = 0; y<N; y++) {
                      points.push(new Vector2f( (0.5 + x) / N * size, (0.25 + 0.5 * x % 2 + y) / N * size ));
                  //new Point());
          }
          }
          return points;
        ...
        private GetBounds(mapW: number, mapH: number): Rectf { return new Rectf(0, 0, mapW, mapH); }

        
        public static GeneratePointsJittered(numCells: number, mapWidth: number, mapHeight: number, r: Random) :Vector2f[]
        {
        ...
        static getBoundaryPoints(width: number, height: number, spacing: number) : Vector2f[]
        {
            ...
            for (let i = 0.5; i < numberX; i++)
            {
                ...
                points.push(new Vector2f(x, offset)); points.push(new Vector2f(x, h + offset));
            }
            for (let i = 0.5; i<numberY; i++)
            {
                var y = Math.ceil(h * i / numberY + offset);
                points.push(new Vector2f(offset, y)); points.push(new Vector2f(w + offset, y));
            }
            return points;
        }

        // get points on a regular square grid and jitter them a bit
        static getJitteredGrid(width: number, height: number, spacing: number, r: Random) : Vector2f[]
        {
            ...
            for (var y = radius; y < height; y += spacing)
            {
                for (var x = radius; x < width; x += spacing)
                {
                    ...
                    points.push(new Vector2f(xj, yj));
                }
            }
            return points;
        }
        ...
        public static rn(v: number, d: number = 0)
        {
            var m = Math.pow(10, d);
